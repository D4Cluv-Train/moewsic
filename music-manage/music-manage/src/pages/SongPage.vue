<template>
  <div class="table">
    ...
    <!-- 修改后的添加歌曲对话框 -->
    <el-dialog title="添加歌曲" :visible.sync="centerDialogVisible" width="400px" center>
      <el-form :model="registerForm" id="tf" enctype="multipart/form-data">
        <div>
          <label>歌曲名</label>
        ...
        <div>
          <label>歌曲上传</label>
          <br>
          <input type="file" name="file" id="update-file-input" @change="handleFileChange">
        </div>
      </el-form>
      <span slot="footer" class="dialog-footer">
        <el-button @click="centerDialogVisible = false">取 消</el-button>
...
export default {
  ...
  methods: {
    ...
    handleFileChange(e) {
      this.registerForm.file = e.target.files[0]; // 保存文件对象
    },
    addSong() {
      // 创建 FormData
      ...
      // 添加表单数据
      formData.append('name', this.singerName + '-' + this.registerForm.name);
      formData.append('singerId', this.singerId);
      formData.append('introduction', this.registerForm.introduction);
      formData.append('lyric', this.registerForm.lyric || '[00:00:00]暂无歌词');

      // 添加文件
      if (this.registerForm.file) {
        formData.append('file', this.registerForm.file);
      } else {
        this.notify('请选择歌曲文件', 'error');
        return;
      }
      ...
      HttpManager.uploadSong(formData)
        .then(res => {
          if (res.code === 1) {
            ...
            this.notify(res.msg || '上传失败', 'error');
          }
        })
        .catch(err => {
          console.error(err);
          this.notify('上传出错', 'error');
        });
