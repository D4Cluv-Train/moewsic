...
const HttpManager = {
  ...
  // =======================> 推荐 API
  // 协同过滤推荐
  getCollaborativeRecommend: (userId, limit) => get(`recommend/collaborative?userId=${userId}&limit=${limit}`),
  // 基于内容的推荐
  getContentRecommend: (userId, limit) => get(`recommend/content?userId=${userId}&limit=${limit}`),
  // 热门歌曲
  getPopularSongs: (limit) => get(`recommend/popular?limit=${limit}`),
  // AI对话
  sendChatMessage: (userId, message) => post(`chat/message`, { userId, message })
}
